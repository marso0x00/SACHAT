...
class User
{
    ...
    public function get_users($query, $skipped_array = [], $mentioned = false)
    {
        ...
        if ($skipped_array) {
            ...
            $skipped_list = $this->spread_ids($skipped_array);
            /* get users */
    ...
    public function scraper($url)
    {
        ...
        if ($embed) {
            ...
            if ($embed->type == "link" || $embed->providerName == "Odysee" || ($embed->type == "rich" && !in_array($embed->providerName, ["Twitter", "SoundCloud"]))) {
    ...
    public function get_posts($args = [])
    {
        ...
        $offset *= $system['newsfeed_results'];
        ...
        if ($last_post_id != null && $get != 'popular' && $get != 'saved' && $get != 'memories') { /* excluded as not ordered by post_id */
            ...
            $limit_statement = ($get_all) ? "" : sprintf("LIMIT %s, %s", secure($offset, 'int', false), secure($system['newsfeed_results'], 'int', false)); /* get_all for cases like download user's posts */
